...
export function useVideoExtractor() {
  ...
  const extractFrames = useCallback(
    async (video: HTMLVideoElement, settings: ExtractionSettings) => {
      ...
      const endFrameIndex =
        settings.endTime > 0 ? Math.floor(settings.endTime / settings.interval) : totalFrames;
      ...
      for (let i = startFrameIndex; i <= endFrameIndex; i++) {
        ...
        const blob = await new Promise<Blob>((resolve) => {
          canvas.toBlob((b) => resolve(b!), settings.format, settings.quality / 100);
        });
