...
import type { ExtractionSettings as ExtractionSettingsType } from '@/shared/lib/hooks/useVideoExtractor';
...
export function ExtractionSettings({
  ...
  const estimatedFrames = rangeDuration > 0
    ? Math.floor(rangeDuration / effectiveInterval) + 1
    : 0;
      ...
      <div className="space-y-3">
        <div className="flex justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {formatTime(timeRange[0])} - {formatTime(timeRange[1])} ({Math.round(timeRange[1] - timeRange[0])}s)
          </span>
        </div>
        <Slider
          value={timeRange}
